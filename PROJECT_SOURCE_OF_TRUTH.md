...
## Compatibility Layer
The repository currently contains legacy root-level files (`index.tsx`, `score.ts`, `router.tsx`, `start.ts`, `server.ts`) that are bridged by `src/` route and module wrappers.
...
- `breadcrumb (1).tsx`
- `utils (1).ts`
- Root-level legacy app files mirrored by `src/` bridges
